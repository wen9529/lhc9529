...
export class PredictionEngine {
  ...
  static generate(history: DbRecord[], type: LotteryType): PredictionData {
    ...
    // --- 🚨 修复头数计算逻辑 🚨 ---
    // 之前错误: Array.from(hSet).sort() 是按数字大小(0,1,2...)排序，导致永远出0头/1头
    // 修正: 统计18码中各头数出现的频率(密度)，按密度倒序排列
    ...
    // 按数量倒序 (b - a)
    const recHeads = Object.keys(headCounts)
        .sort((a, b) => headCounts[parseInt(b)] - headCounts[parseInt(a)])
        .slice(0, 2);
